...
def read_midi_file(midi_path, max_iters=-1):
    ...
    metadata = dict()
    metadata['charset'] = mid.charset
    metadata['clip'] = mid.clip
    metadata['length'] = mid.length
    metadata['ticks_per_beat'] = mid.ticks_per_beat
    metadata['tracks'] = [len(t) for t in mid.tracks]
    metadata['type'] = mid.type
    ...
    for msg in mid:
        ...
        # DEBUG:
        # if cur_iters <= 100 and np.random.rand() < 0.1:
        #     print(msg)
    ...
    print(f'read_midi_file() took {passed:.3f}s')

    return (event_seq, metadata)
