...
class MyTrainPipeline(torch.nn.Module):
    ...
    def get_normalized_values(self,analysis_output):
        carrier_weight = analysis_output[:,:,:127]
        mod_weight = analysis_output[:,:,127:127*2]
        carrier_slope = analysis_output[:,:,127*2:127*3]
        carrier_offset = analysis_output[:,:,127*3:127*4]
        mod_slope = analysis_output[:,:,127*4:127*5]
        mod_offset = analysis_output[:,:,127*5:127*6]
        phase_offset = analysis_output[:,:,127*6:127*7]
        test = analysis_output[:,:,127*7:]
        self.min_slope=-2.0
        self.max_slope=8.0

        carrier_weight= F.softmax(carrier_weight,dim=2)
        mod_weight= F.softmax(mod_weight,dim=2)
        carrier_env_slope = torch.sigmoid(carrier_slope) 
        mod_env_slope = torch.sigmoid(mod_slope) 
        carrier_env_offset = torch.tanh(carrier_offset) 
        mod_env_offset = torch.tanh(mod_offset) 
        phase_offset =torch.sigmoid(phase_offset)
        analysis_o = torch.cat([carrier_weight, mod_weight,carrier_env_slope, carrier_env_offset,mod_env_slope,mod_env_offset,phase_offset,test],dim=2)
        analysis_o = (analysis+1)/2.0
        return analysis_o
    ...
    def forward(self, data_retval, cur_step, total_step):
        ...
        #analysis_o = self.get_normalized_values(analysis_output)
